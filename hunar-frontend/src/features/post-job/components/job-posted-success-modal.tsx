    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in-50 duration-200">
      <div className="bg-white w-full max-w-md rounded-3xl border border-slate-200 shadow-2xl p-6 text-center space-y-5 animate-in zoom-in-95 duration-200">
        ...
        <div className="relative size-20 mx-auto flex items-center justify-center">
          <div className="absolute inset-0 rounded-full bg-[#0F8B8D]/15 animate-ping" />
          <div className="absolute inset-2 rounded-full bg-[#0F8B8D]/20 animate-pulse" />
          <div className="relative size-14 rounded-full bg-[#0F8B8D] text-white flex items-center justify-center shadow-lg">
        ...
        <div className="flex flex-col gap-2 pt-2">
          <Link
            href="/customer/jobs"
            className="w-full py-3 px-4 rounded-xl bg-[#0F8B8D] hover:bg-[#0D7A7C] text-white font-bold text-xs sm:text-sm shadow-sm transition-all active:scale-[0.98] text-center"
          >
            {isUrdu ? "میری جابز اور موصولہ آفرز دیکھیں" : "View in My Jobs & Live Bids"}
          </Link>
          <Link
            href="/customer/dashboard"
            className="w-full py-2.5 px-4 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs transition-colors text-center"
          >
