...
import { Api } from '../shared/http/api-client.js';
import { WORKERS, DEMO_CUSTOMER, initialSeedReviewsFor, avatarColor } from '../modules/users/users.data.js';
import { randDist } from '../modules/locations/locations.data.js';
import { fmtRs, uid, jobId } from '../shared/ui/ui.js';
...
export const Store = {
  ...
  /* MongoDB jobs persistence matching existing contract */
  ...
  currentUser: function () {
    const s = this._state;
    if (!s || !s.currentUserId) return null;
    return s.users.find(function (u) { return u.id === s.currentUserId; }) || null;
  },
  userById: function (id) {
    if (!this._state || !this._state.users) return null;
    return this._state.users.find(function (u) { return u.id === id; });
  },
  ...
  unreadFor: function (userId) {
    const arr = (this._state && this._state.notifications[userId]) || [];
  ...
  reviewsFor: function (workerId) {
    return (this._state && this._state.reviews ? this._state.reviews : []).filter(function (r) { return r.workerId === workerId; });
  },

  earnings: function () {
    const u = this.currentUser();
    const s = this._state;
    if (!u) return { total: 0, monthly: {} };
    let total = 0;
    const byWorker = {};
