...
Wallet live via `PaymentsModule` (schema-aligned Task 7 wallet). The standalone `src/modules/wallet` pre-merge design stays excluded (see note at bottom).
...
|---|------|-------------|--------|
| 1 | Worker sign-up (phone → OTP → password) | `POST /auth/worker/signup` | ✅ |
| 2 | Request OTP | `POST /auth/worker/otp/request` | ✅ |
| 3 | Verify OTP | `POST /auth/worker/otp/verify` | ✅ |
| 4 | Complete signup (set password) | `POST /auth/worker/signup/complete` | ✅ |
| 5 | Worker login (phone + password) | `POST /auth/worker/login` | ✅ |
| 6 | Refresh token | `POST /auth/worker/refresh` | ✅ |
| 7 | Role guard middleware | Middleware | ✅ |
| 8 | Onboarding — save personal details | `PUT /workers/onboarding/personal` | ✅ |
| 9 | Onboarding — save skills | `PUT /workers/onboarding/skills` | ✅ |
| 10 | Onboarding — save experience | `PUT /workers/onboarding/experience` | ✅ |
| 11 | Onboarding — save service areas (PostGIS) | `PUT /workers/onboarding/areas` | ✅ |
| 12 | Onboarding — upload documents | `POST /workers/onboarding/documents` | ✅ |
| 13 | Onboarding — submit for verification | `POST /workers/onboarding/submit` | ✅ |
| 14 | Get verification status | `GET /workers/verification-status` | ✅ |
| 15 | Worker profile (public) | `GET /workers/[id]` | ✅ |
| 16 | Edit profile | `PUT /workers/[id]` | ✅ |
...
| # | Task | API Endpoint | Status |
|---|------|-------------|--------|
| 17 | Get nearby jobs (PostGIS matching) | `GET /jobs/nearby` | ✅ |
| 18 | Get job detail | `GET /jobs/[id]` | ✅ |
| 19 | Send visit offer | `POST /jobs/[id]/offers` | ✅ |
| 20 | Get my offers | `GET /workers/me/offers` | ✅ |
| 21 | Counter offer | `PUT /offers/[id]/counter` | ✅ |
| 22 | Accept counter | `PUT /offers/[id]/accept-counter` | ✅ |
| 23 | Get active jobs | `GET /workers/me/jobs/active` | ✅ |
| 24 | Start visit | `PUT /jobs/[id]/visit/start` | ✅ |
...
| # | Task | API Endpoint | Status |
|---|------|-------------|--------|
| 29 | Repair negotiation (accept/counter) | `PUT /jobs/[id]/repair/counter` | ✅ |
| 30 | Approve repair | `PUT /jobs/[id]/repair/approve` | ✅ |
...
- [x] Onboarding APIs (6 steps incl. skills + service areas + documents)
- [x] Verification workflow (approved/rejected/request-changes + re-submit)
- [x] Jobs feed API with PostGIS ST_DWithin + all 5 filters
- [x] Offers: send (one per job), counter (bounded), accept/reject, real-time close to others
- [x] Visits: state machine + live location tracking
...
is **live via `src/modules/payments`** — `PaymentsController` serves all wallet/commission/earnings endpoints
(`GET /wallet/balance`, `GET /wallet/ledger`, `GET /workers/me/earnings`, `POST /wallet/topup`,
`GET /wallet/topup/status`, `POST /wallet/hold-commission`, `POST /wallet/confirm-commission`,
`POST /wallet/reverse-commission`, `GET /admin/wallet/platform`). Wallet tests pass:
`src/modules/payments/wallet.service.spec.ts`.

The standalone `src/modules/wallet` module is a pre-merge design that no longer matches the merged Task 7 schema
(`workerId`-keyed wallets, `walletTopUp` + `paymentMethod`, HELD/DEDUCTED/REVERSED commission statuses). It stays
**excluded from build + tests** (`tsconfig.json`, jest `testPathIgnorePatterns`) and is **not registered** to avoid
duplicate routes/events. `PaymentsModule` is the single wallet owner.
